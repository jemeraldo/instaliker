...
def like_medias(self, medias, quiet=True):
    broken_items = []
    if not medias:
...
def like_hashtag(self, hashtag, amount=None, quiet=True):
    """ Likes last medias from hashtag """
    self.logger.info("Going to like media with hashtag #%s" % hashtag)
    medias = self.get_total_hashtag_medias(hashtag, amount=80, filtration=True)
    return self.like_medias(medias[:amount], quiet=quiet)


def like_geotag(self, geotag, amount=None, quiet=True):
    self.logger.info("Going to like media with geotag %s" % geotag)
    medias = self.get_total_geotag_medias(geotag, amount=80)
    return self.like_medias(medias[:amount], quiet=quiet)
